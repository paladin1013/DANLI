...
if __name__ == "__main__":
    ...
    result_data = []
    for idx, edh_sample_file in enumerate(edh_samples):
        sample_data = {}
        edh_raw, edh_text = edh_file_loader(edh_sample_file)


        
        print(f"\n================== Sample {idx} {edh_sample_file} ===================\n")
        ...
        # result_data.append({"game_id":game_id, "edh_num":edh_num, "subgoals_groundtruth":gt_subgoals, "subgoals_predicted":future_subgoals, "gpt_prompt": prompt, "gpt35_reply_raw": replies[0], "gpt_reply_formatted": replies[1]})
        result_data.append(sample_data)
        
    with open(f"{GPT_RESULT_ROOT}/sg_prediction_seed{SEED}.json", "w") as f:
